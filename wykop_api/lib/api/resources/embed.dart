...
class EmbedApi extends ApiResource {
  ...
  Future<String> getGfycatUrl(String url) async {
    ...
    var gfyId = gfycatItem[url.endsWith('/') ? gfycatItem.length - 2 : gfycatItem.length - 1];
  ...
  Future<String> getStreamableUrl(String url) async {
    ...
    var streamableId = streamableItems[url.endsWith('/') ? streamableItems.length - 2 : streamableItems.length - 1];
    var data = await http.get('https://api.streamable.com/videos/' + streamableId);
